...
$usePint = confirm('Enable Pint?', false);
$useDependabot = confirm('Enable Dependabot?', false);
$useLaravelRay = confirm('Enable Ray?', false);
...
if (confirm('Let this script delete itself?', false)) {
    unlink(__FILE__);
}
...
function ask(string $question, string $default = ''): string
{
    ...
    echo ("\e[0;32m".$question.$def.": \e[0m");
    $answer = readline();
...
function removeNpmDeps(array $names, string $location): void
{
    ...
    file_put_contents(__DIR__.'/package.json', json_encode($data, JSON_PRETTY_PRINT
